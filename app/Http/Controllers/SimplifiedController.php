...
use App\Exports\ExternalSimplifiedExport;
...
class SimplifiedController extends Controller
{
    public function index(Request $request)
    {
        ...
        $branches = Branch::all();
        return view('site.simplified.index', compact('data', 'branches'));
    }
    ...
    public function create()
    {
        ...
        $step = SimplifiedInvoice::where('supervisor_id', 1)
            ->where('status', 'open')
            ->first();
        if (!empty($step)) {
            $open_invoice = $step;
        ...
        return view('site.simplified.create',
    ...
    public function export_simplified()
    {
        return Excel::download(new ExternalSimplifiedExport(), 'كل الفواتير الضريبية المبسطة.xlsx');
    }
    ...
    public function print($id)
    {
        ...
        if (!empty($simplified)) {
            ...
            if ($elements->isEmpty()) {
                ...
                return view('site.simplified.print', compact('simplified'));
            }
        } else {
            return abort('404');
        }
